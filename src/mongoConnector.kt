...
class MongoDriver(mongoClient: MongoClient, db: String) {
    private var defaultDb = "dev-user-db";
    ...
    fun addNewDocument(col: String, doc: JsonString): String? {
        try {
    ...
    fun deleteFromCollection(collection: String, id: String?): Pair<Int, String> {
        if (!ObjectId.isValid(id)) {
            return Pair(0, "Yo that ID ain't real!")
        }
        val filter = BsonDocument("_id", BsonObjectId(ObjectId(id)))
